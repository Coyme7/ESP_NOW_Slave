...
void printSlaveCurrentDiag() {
    ...
    Serial.printf("  x: motor=%u sense=%u raw=%d,%d adc_err=%lu/%u offset=%.4f/%.4fV iq/id=%.4f/%.4fA vq/vd=%.3f/%.3fV\n",
                  ...
                  current.x.raw_adc_a,
                  current.x.raw_adc_b,
                  static_cast<unsigned long>(current.x.adc_read_errors),
                  static_cast<unsigned int>(current.x.adc_consecutive_errors),
    ...
    Serial.printf("  y: motor=%u sense=%u raw=%d,%d adc_err=%lu/%u offset=%.4f/%.4fV iq/id=%.4f/%.4fA vq/vd=%.3f/%.3fV\n",
                  current.y.motor_ready ? 1 : 0,
                  current.y.current_sense_ready ? 1 : 0,
                  current.y.raw_adc_a,
                  current.y.raw_adc_b,
                  static_cast<unsigned long>(current.y.adc_read_errors),
                  static_cast<unsigned int>(current.y.adc_consecutive_errors),
